...
```{r}
# linear models by census region
census <- recs %>% pull(census_div) %>% unique()
census_mod_results <- vector(mode = "list", length = length(census))
names(census_mod_results) <- census

for (i in seq_along(census)) {
    this_region <- census[i]
    recs_sub <- recs %>%
        filter(census_div==this_region)
    # The double square brackets [[i]] accesses the ith element of a list
    census_mod_results[[i]] <- lm(TOTALBTU ~ TOTSQFT_EN, data = recs_sub)
}

purrr::map(census_mod_results, summary)
...
```{r fig-housing-type}
#| fig-cap: "Average energy use per household for each housing type, both total and per unit area. Although total energy use increases as density decreases, energy use per square foot shows less variance."

# median btu per housing type
avg_energy <- recs %>% 
