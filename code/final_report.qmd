...
color_tot = "forest green"
color_area = "light blue"
...
avg_energy %>% 
  ...
  geom_point(aes(y = avg_energy_use), color = color_tot, shape = 18,
             size = 3) +
  geom_point(aes(y = avg_energy_per_sqft / coeff), color = color_area) +
  scale_y_continuous(
...
```
	...
	The crux of the matter is that if we want to use less energy, we’re going to have to learn to take up less space. As cities grow, this becomes more and more essential, for a lot of compounding reasons - avoiding sprawl, increasing housing capacity, improving transit and quality of life. This data makes it clear that energy is another strogn factor in that list.


## Geographic Trends

Since square footage seems to be the biggest indicator of residential energy usage, let's look more closely at housing size across the country. States in the Midwest tend to have the largest homes, followed by New England. Since RECS data was collected to be compatible with the ACS, we can zoom into the state level of some RECS variables using ACS data. Here, we find that states in those areas, particularly New York, Massachusetts, New Jersey, and Illinois also tend to have older buildings. Houses in the South and West generally tend to be both newer and smaller. While this is all highly aggregated data, it could indicate a trend towards smaller residential buildings over time.

[figure: Median Building Age and Square Footage by Region]

Looking at size by number of units, single-unit detached homes dominate overall, although less so in New England. This is likely due to the presence of more compact cities like New York City and denser living zones overall. Single unit houses, unsurprisingly, also dominate by state. They seem least prevalent in states containing large metropolitan areas. Apartments and attached homes are relatively common in New England. Over a fifth of Rhode Island's housing is small apartments; New York, Connecticut, and New Jersey also have high proportions. Although not visible, around half of D.C.'s housing is large apartments, followed by New York, supporting the idea that cities tend to contain larger unit housing.

[figure: state-level maps of housing unit proportions]

Based on previous findings, we would expect Midwestern states to use more energy, and they do use more natural gas than other regions–likely for heating. The South uses noticeably more air conditioning than other regions. Overall, New England uses the most energy, followed by the Midwest.
